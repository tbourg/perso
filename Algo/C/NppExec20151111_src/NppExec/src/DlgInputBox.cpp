  ...
  LPARAM lParam)
{
    if (uMessage == WM_COMMAND)
    {
        switch (LOWORD(wParam)) 
        {
            ...
            case IDC_BT_TERMINATE:
            {
                EndDialog(hDlg, CInputBoxDlg::RET_TERMINATE);
                return 1;
            }
            default:
                break;
...
{ 
    ...
    switch ( m_type )
    {
        case IBT_INPUTBOX:
            ...
            m_btTerminate.ShowWindow(SW_HIDE);
        ...
        case IBT_EXITPROCESS:
            ...
            ::ScreenToClient(m_hWnd, (POINT *) &rc);
            m_btOK.MoveWindow(rc.left, rc.top);
            ::GetWindowRect(m_btTerminate.m_hWnd, &rc);
            rc.left = m_nScndBtnLeftPos;
            ::ScreenToClient(m_hWnd, (POINT *) &rc);
            m_btTerminate.MoveWindow(rc.left, rc.top);
            m_btOK.SetWindowText( _T("Send") );
            m_btOK.ShowWindow(SW_SHOW);
            m_btTerminate.ShowWindow(SW_SHOW);
...
void CInputBoxDlg::OnInitDialog(HWND hDlg)
{
    ...
    m_btTerminate.m_hWnd = ::GetDlgItem(hDlg, IDC_BT_TERMINATE);
    ...
    if ( msg.SplitAsArgs(m_InputMessage, _T(':'), 2) == 2 )
    {
             ...
             (valueSpecs.SplitAsArgs(initialValue, _T(':'), 2) == 2) )
        {
            ...
            if ( (m_InputVarName.GetFirstChar() == _T('\"')) && 
                 (m_InputVarName.GetLastChar() == _T('\"')) )
            {
                m_InputVarName.DeleteFirstChar();
                m_InputVarName.DeleteLastChar();
            }
    ...
    {
        if ( m_InputMessage.GetFirstChar() == _T('\"') )
            m_InputMessage.DeleteFirstChar();
        if ( m_InputMessage.GetLastChar() == _T('\"') )
            m_InputMessage.DeleteLastChar();
