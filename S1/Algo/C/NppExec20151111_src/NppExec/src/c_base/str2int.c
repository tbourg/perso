    ...
    *sign = 0;
    ...
    if (strw[0] == L'0')
    {
        switch (strw[1])
        {
            ...
            {
                // HEX value
                return strbase2uintw(strw + 2, 16);
            }
            case L'b': // "0b..."
            case L'B': // "0B..."
            {
                // BIN value
                return strbase2uintw(strw + 2, 2);
            }
        #if LEADING_00_IS_DEC_VALUE
            case L'0': // "00..."
            {
                // DEC value with leading zeros
                return strbase2uintw(strw + 2, 10);
            }
        #endif
            default: // "0..."
            {
                // OCT value
                return strbase2uintw(strw + 1, 8);
            }
        }
    }
    if (strw[0] == L'$')
    {
        // HEX value
        return strbase2uintw(strw + 1, 16);
    }

    // DEC value
    return strbase2uintw(strw, 10);
}
